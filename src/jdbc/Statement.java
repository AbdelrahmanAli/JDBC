...
public class Statement implements java.sql.Statement
{
	...
	private jdbc.ResultSet resultSet;
	...
	public jdbc.ResultSet executeQuery(String sql) throws SQLException 
	{
		if(isClosed) {
			System.out.println("The Statement is closed.");
			return null;
        ...
        if(query.getType()==5){
            ...
            fillRecordsArray(records ,resultTable);
            resultSet = new jdbc.ResultSet(columnNameToIndex,IndexToColumnType,records,dbms.propArray(),this);
            return  resultSet;
        }else   return null;
	}
 
	public void fillRecordsArray(String [][] records ,ArrayList<ArrayList<String> > resultTable)
	{
		for(int i = 0 ; i < records.length ; i++) // for each row , for each column
        {	for(int j = 0 ; j < records[0].length ; j++)
        	{
        		if(resultTable.get(i+1).get(j).equalsIgnoreCase("empty")) records[i][j] = null;
        		else records[i][j] = resultTable.get(i+1).get(j); 
        	}	
        }
	}
