...
public class ResultSetMetaData implements java.sql.ResultSetMetaData {
	...
	Logger log = Logger.getLogger(ResultSetMetaData.class);
	
	public ResultSetMetaData(jdbc.ResultSet rs){
		this.columnNameToIndex = rs.getColumnNameToIndex();
		this.columnNameToIndex = rs.getColumnNameToIndex();
		this.prop = rs.getProp().clone();
	}
	...
	public String getColumnLabel(int column) throws SQLException {
		...
		for(int i = 0; i<a.size(); i++){
			if(columnNameToIndex.get(temp[i])==(column-1)){
				testResult = (String)temp[i];
				return testResult;
			}
		}
		log.error("Column index is not exist.");
	...
	public int getColumnType(int column) throws SQLException {
		int result = getBaseType(IndexToColumnType.get(column-1));
